...
func Test_NewEnvironmentStartCommand(t *testing.T) {
	f := &factory.Factory{}
	...
	t.Run("CommandCreation", func(t *testing.T) {
		cmd := NewEnvironmentStartCommand(&o)

		assert.NotNil(t, cmd, "Expect command to be not nil")
		assert.Equal(t, []string{"sta", "b", "boot"}, cmd.Aliases, "Expect aliases to be equal")
	...
	t.Run("CommandName", func(t *testing.T) {
		...
		assert.Equal(t, "start", cmd.Use, "Expected command name to be 'start'")
	})
	...
	tt := []struct {
		...
		{
			name: "help",
			out:  grammar.EnvironmentStartHelp,
			args: []string{"--help"},
			err:  "",
		},
		{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			o := Opts{F: f, I: interfaceRecorder}
			f.LoggedIn = false
			startCmd := NewEnvironmentStartCommand(&o)

			startCmd.SetArgs(tc.args)

			stdin := new(bytes.Buffer)
			stderr := new(bytes.Buffer)

			startCmd.SetIn(stdin)
			startCmd.SetOut(stderr)
			startCmd.SetErr(stderr)

			_, err := startCmd.ExecuteC()
			fmt.Println(err)
			if tc.err != "" {
...
func Test_StartRunner(t *testing.T) {
	f := mock_factory.NewFactory()
	f.LoggedIn = true
	...
	t.Run("command initialization with prerunner", func(t *testing.T) {
		o := Opts{F: f, I: interfaceRecorder}
		expected := NewEnvironmentStartCommand(&o)
		mustEqual := expected
		interfaceRecorder.EXPECT().StartRunner(expected, []string{}).Times(1)
	...
	t.Run("both id and name", func(t *testing.T) {
		...
		o := Opts{F: f, I: interfaceRecorder}
		_ = NewEnvironmentStartCommand(&o)
		o.EnvID = "123"
		o.EnvName = "abc"
		o.StartRunner(&cobra.Command{}, []string{})
	})
	...
	t.Run("get by id", func(t *testing.T) {
		o := Opts{F: f, I: interfaceRecorder}
		o.EnvID = "1"
		interfaceRecorder.EXPECT().StartEnvByID()

		o.StartRunner(&cobra.Command{}, []string{})
	})
}

func Test_StartEnvByID(t *testing.T) {
	f := mock_factory.NewFactory()
	f.LoggedIn = true

	requester.Client = &mock.Client{}
	mock.GetDoFunc = mock.StartEnvironment
	...
	recorder.EXPECT().Println("Sent command for starting environment")
