...
func TestDisableProjectCmd(t *testing.T) {
	t.Run("CommandCreation", func(t *testing.T) {
		f := mock_factory.NewFactory()
		cmd := NewProjectDisableCommand(f)

		assert.NotNil(t, cmd, "Expect command to be not nil")
		assert.Equal(t, []string{"d", "dis", "disab"}, cmd.Aliases, "Expect aliases to be equal")
	...
	t.Run("CommandName", func(t *testing.T) {
		...
		assert.Equal(t, "disable", cmd.Use, "Expected command name to be 'disable'")
	})
	...
	tt := []struct {
		...
		{
			name: "help",
			out:  grammar.ProjectDisableHelp,
			args: []string{"--help"},
			err:  "",
		},
		{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			f := mock_factory.NewFactory()
			f.LoggedIn = false
			disableCmd := NewProjectDisableCommand(f)

			disableCmd.SetArgs(tc.args)

			stdin := new(bytes.Buffer)
			stderr := new(bytes.Buffer)

			disableCmd.SetIn(stdin)
			disableCmd.SetOut(stderr)
			disableCmd.SetErr(stderr)

			_, err := disableCmd.ExecuteC()
			if tc.err != "" {
...
func TestDisableByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			id:        "123abc",
			getDoFunc: mock.DisableProjectByID,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.name == "Success" {
				recorder.EXPECT().Print(f.IO.ColorScheme().SuccessIcon(), " Successfully deactivated project.")
			}

			opts := &Opts{F: f}
			opts.ID = test.id
			opts.F.LoggedIn = true
			opts.disable(&cobra.Command{}, []string{})
		})
	}
}

func TestDisableBothIDAndname(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	recorder := mock_printer.NewMockPrinterInterface(ctrl)
	f := mock_factory.NewFactory()
...
func TestDisableByName(t *testing.T) {
	requester.Client = &mock.Client{}
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	requester.Client = &mock.Client{}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// 		mock.GetDoFunc = mock.DisableProjectByID
			// 	}()
			// }
			test.expect(recorder)
			mock.GetDoFunc = test.getDoFunc
